...
class RustCodegen:
    ...
    def generate(self, ir_mod: IRModule) -> str:
        ...
        if stmt_lines:
            final_lines.append("fn main() {")
            final_lines.append("    if let Err(e) = _main() {")
            final_lines.append("        eprintln!(\"Py2Rust Error: {}\", e);")
            final_lines.append("        std::process::exit(1);")
            final_lines.append("    }")
            final_lines.append("}")
            final_lines.append("")
            final_lines.append("fn _main() -> Result<(), PyError> {")
