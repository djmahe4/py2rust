...
class RustCodegen:
    ...
    def generate(self, ir_mod: IRModule) -> str:
        self._output = []
        self._emit("// Generated by py2rust")
        self._emit("use std::collections::HashMap;")
        self._emit("")
        self._emit("#[derive(Debug, Clone)]")
        self._emit("pub enum PyError {")
        self._indent += 1
        self._emit("Exception(String),")
        self._emit("ValueError(String),")
        self._emit("TypeError(String),")
        self._emit("KeyError(String),")
        self._emit("IndexError(String),")
        self._indent -= 1
        self._emit("}")
        self._emit("")
        self._emit("impl std::fmt::Display for PyError {")
        self._indent += 1
        self._emit("fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {")
        self._indent += 1
        self._emit("match self {")
        self._indent += 1
        self._emit('PyError::Exception(s) => write!(f, "Exception: {}", s),')
        self._emit('PyError::ValueError(s) => write!(f, "ValueError: {}", s),')
        self._emit('PyError::TypeError(s) => write!(f, "TypeError: {}", s),')
        self._emit('PyError::KeyError(s) => write!(f, "KeyError: {}", s),')
        self._emit('PyError::IndexError(s) => write!(f, "IndexError: {}", s),')
        self._indent -= 1
        self._emit("}")
        self._indent -= 1
        self._emit("}")
    ...
    def _default_value(self, ir_type) -> str:
        ...
        if isinstance(ir_type, IRClassType):
            return f"{_rust_type(ir_type)}::new()"
    ...
    def _gen_stmt(self, stmt) -> None:
        if isinstance(stmt, IRVarDecl):
            ...
            if stmt.name == "_":
                ...
                self._emit(
                    f"let mut {_mangle(stmt.name)}: {_rust_type(stmt.type_)} = {val};"
                )
            else:
                self._emit(
                    f"let {_mangle(stmt.name)}: {_rust_type(stmt.type_)} = {val};"
                )
